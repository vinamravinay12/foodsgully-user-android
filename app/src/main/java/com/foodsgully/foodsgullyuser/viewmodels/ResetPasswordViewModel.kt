...
import com.foodsgully.foodsgullyuser.viewmodels.repositories.ResetPasswordRepository
...
class ResetPasswordViewModel : ViewModel() {
    ...
    private fun validateAllFields(context: Context) : Boolean {
        return validateEmail(context) && validatePassword(context)
    }

    fun resetPassword(context: Context) : MutableLiveData<GenericResponse>? {

        if(!validateAllFields(context)) return null
        return ResetPasswordRepository<String>(LoginData(userName = email.value ?: "", password = password.value ?: ""),context).getResponse()
    }
}