...
class SearchProductsRepository<T>(private val query : String,context: Context) : Repository(context) {
    ...
    override fun getResponse(): MutableLiveData<GenericResponse> {
        ...
        getAPIInterface()?.searchProducts(query)?.enqueue(object :
            Callback<GenericAPIResponse<List<Product>>> {
